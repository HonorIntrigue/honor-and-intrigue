export default class HonorIntrigueProtectionRoll extends foundry.dice.Roll {
  ...
  static async prompt(options = {}) {
    ...
    options.protectionItems = options.actor.itemTypes.armor
      .filter((item) => item.system.protection.value)
      .map((item) => ({
        ...item,
        id: item.id,
    ...
    if (options.protectionItems.length === 0) {
      ui.notifications.warn(
        game.i18n.format('HONOR_INTRIGUE.Dialog.Result.NoProtection', { name: options.actor.name }),
      );
      return false;
    }
    ...
    return {
      protectionItems: Object.fromEntries(
        items.map((curr) => [curr.id, { formula: curr.protection, name: curr.name }]),
      ),
      rollMode,
      rolls: [roll],
    };
