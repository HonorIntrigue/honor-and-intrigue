...
export default class CharacterActorSheet extends ManeuverSupportMixin(HonorIntrigueActorSheet) {
  ...
  async _renderHTML(context, options) {
    const img =
      this.actor.system.advantage === 0
        ? 'advantage_defeated'
        : this.actor.system.advantage === 1
          ? 'advantage_scrambling'
          : this.actor.system.advantage === 2
            ? 'advantage_retreating'
            : 'advantage_en-garde';
    const htmlString = await foundry.applications.handlebars.renderTemplate(
      systemPath('templates/sheets/actor/partials/advantage-panel.hbs'),
      {
