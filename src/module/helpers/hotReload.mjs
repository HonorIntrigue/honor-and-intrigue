...
export function hotReload(data) {
  ...
  } else if (data.path.includes(systemPath('css'))) {
    let path = systemPath('css/honor-and-intrigue');
    if (data.path.includes('-elements')) path += '-elements.css';
    else if (data.path.includes('-variables')) path += '-variables.css';
    else path += '.css';

    // Taken from core's `Game##hotReloadCSS`
    const pathRegex = new RegExp(`@use "${path}(?:\\?[^"]+)?"`);
    for (const style of document.querySelectorAll('style')) {
      const [match] = style.textContent.match(pathRegex) ?? [];
      if (match) {
        style.textContent = style.textContent.replace(match, `@use "${path}?${Date.now()}"`);
        return;
      }
    }
  }
