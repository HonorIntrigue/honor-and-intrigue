...
export default class QualityRollMessageModel extends BaseMessageModel {
  ...
  static defineSchema() {
    ...
    schema.quality = new fields.SchemaField({
      key: new fields.StringField({
        choices: [...Object.values(hi.CONFIG.qualities), ...Object.values(hi.CONFIG.shipQualities)].map(
          (v) => v.rollKey,
        ),
        nullable: false,
        required: true,
    ...
    schema.statusModifiers = new fields.TypedObjectField(
  ...
  async alterMessageHTML(html) {
    ...
    html
      .querySelector('.message-header .flavor-text')
      .insertAdjacentHTML(
        'beforeend',
    ...
    const mods = [
      game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.Ability', {
        ability: game.i18n.localize(
          (hi.CONFIG.qualities[this.quality.key] ?? hi.CONFIG.shipQualities[this.quality.key]).label,
        ),
        number: this.quality.value.signedString(),
      }),
    ];

    if (this.modifiers.combatAbility?.value)
      mods.push(
        game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.Ability', {
    ...
    if (this.modifiers.career?.value)
      mods.push(
        game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.Ability', {
    ...
    if (this.modifiers.bonuses > 0)
      mods.push(game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.BonusDice', { number: this.modifiers.bonuses }));
    if (this.modifiers.penalties > 0)
      mods.push(
        game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.PenaltyDice', { number: this.modifiers.penalties }),
      );
    if (this.modifiers.flatModifier !== 0)
      mods.push(
        game.i18n.format('HONOR_INTRIGUE.Chat.Roll.Modifier.Flat', {
          number: this.modifiers.flatModifier.signedString(),
        }),
      );

    const details = await foundry.applications.handlebars.renderTemplate(
      systemPath('templates/rolls/quality-roll-content.hbs'),
      {
        modifiers: mods,
        statusModifiers: Object.values(this.statusModifiers).map(({ label, value }) =>
