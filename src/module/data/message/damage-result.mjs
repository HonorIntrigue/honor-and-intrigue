...
export default class DamageResultMessageModel extends BaseMessageModel {
  ...
  static defineSchema() {
    ...
    schema.protectionItems = new fields.TypedObjectField(
  ...
  async alterMessageHTML(html) {
    ...
    if (this.protectionItems) {
      html.querySelector('.message-content').insertAdjacentHTML(
        'afterbegin',
        await foundry.applications.handlebars.renderTemplate(systemPath('templates/rolls/protection-roll-footer.hbs'), {
          protectionItems: Object.entries(this.protectionItems).map(([id, v]) => ({ id, ...v })),
        }),
      );
    }
  }
